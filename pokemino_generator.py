...
class Pokemino ():
    ...
    def __init__ (self, seed, size, volume, pos=-1, density=1, algorithm='clumped'):
        ...
        if algorithm == 'clumped':
            ...
            while np.count_nonzero(self.bricks) < self.size:
                ...
                for brick in bricks_pos:
                    for step in STEPS:
                        if tuple(np.array(brick) + np.array(step)) not in bricks_pos:
                            if tuple(np.array(brick) + np.array(step)) not in excluded_pos:
    ...
    def visualise_in_napari (self, display_window_size):
        ...
        viewer = napari.view_image(display_window)

class Pokemino2D (Pokemino):
    
    def __init__ (self, seed, size, volume, positioning, density = 1, algorithm="clumped"):
        self.dim = 2
    ...
    def rotate_the_block(self, theta=None, randomise=True):
        ...
        if randomise:
            ...
            assert (isinstance(theta, (int, float))), 'Error: Pokemino2D.rotate_the_block requires the value for theta that is int or float.'
...
class Pokemino3D (Pokemino):
    ...
    def rotate_the_block_1_axis(self, axis=None, theta=None, randomise=True):
        ...
        if randomise:
            ...
            assert (isinstance(theta, (int, float))), 'Error: Pokemino3D.rotate_the_block requires the value for theta that is int or float.'
            theta=theta/180*np.pi
...
class Volume ():
    ...
    def fit_excluded_volume (self, pokemino):
        for excluded_voxel in pokemino.excluded_pos:
            ...
            if all([(coord >= 0) and (coord < self.shape[i]) for i, coord in enumerate(placement_coords)]):
                self.array[placement_coords] = 0.5
    ...
    def check_for_pairwise_overlap (self, fit_excluded_volume=False):
        overlap = True
        while overlap == True:
            overlap = False
    ...
    def add_extra_volume_to_remove_cuts(self):
        ...
        for creature in self.creatures:
            all_positions = np.vstack([all_positions, np.array([brick.pos for brick in creature.bricks]) + creature.positioning])
    ...
    def create_subpixel_resolution_array (self, subpixels):
        ...
        if self.new_image is None:    
            self.subpixel_resolution_array = np.zeros(tuple([i*subpixels for i in self.shape]), dtype=np.int8)
            # TODO: replace with generators???
            for x in range(self.shape[0]):
                for y in range(self.shape[1]):
                    for z in range(self.shape[2]):
                        self.subpixel_resolution_array[subpixels*x:subpixels*(x+1), 
                                                       subpixels*y:subpixels*(y+1), 
                                                       subpixels*z:subpixels*(z+1)] = self.array[x][y][z]
        else:
            self.subpixel_resolution_array = np.zeros(tuple([i*subpixels for i in self.new_image.shape]), dtype=np.int8)
            for x in range(self.new_image.shape[0]):
                for y in range(self.new_image.shape[1]):
                    for z in range(self.new_image.shape[2]):
                        self.subpixel_resolution_array[subpixels*x:subpixels*(x+1), 
                                                       subpixels*y:subpixels*(y+1), 
                                                       subpixels*z:subpixels*(z+1)] = self.new_image[x][y][z]
    
    # def describe_the_volume(self):
    ...
    def display_in_napari(self):
        if self.subpixel_resolution_array is None:
            ...
            viewer = napari.view_image(self.subpixel_resolution_array)